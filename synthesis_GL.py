...
usage: synthesis_GL.py [options] <checkpoint> <text_list_file> <dst_dir>
...
if __name__ == "__main__":
    ...
    dst_mel_path = join(dst_dir, "{}{}_mel.npy".format(checkpoint_name,
                                                          file_name_suffix))
    mels = []
...
<div align="center"><img src="/audio/{}/{}/{}" /></div>
    ...
    mels = np.array([_pad_2d(x, max_mel_length) for x in mels], dtype=np.float32)

    mels = np.stack(mels)
    np.save(dst_mel_path, mels)
